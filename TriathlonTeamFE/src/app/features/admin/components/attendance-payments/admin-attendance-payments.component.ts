...
import { AdminApiService, WeeklyCalendar } from '../../services/admin-api.service';
import { AttendanceModalComponent, AttendanceModalData } from './attendance-modal.component';
import { WebSocketService } from '../../../../core/services/websocket.service';

@Component({
  selector: 'app-admin-attendance-payments',
  standalone: true,
  imports: [
    CommonModule,
    MatButtonModule,
  ...
  templateUrl: './admin-attendance-payments.component.html',
  styleUrls: ['./admin-attendance-payments.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class AdminAttendancePaymentsComponent implements OnInit {
  private readonly api = inject(AdminApiService);
  private readonly dialog = inject(MatDialog);
  private readonly snackbar = inject(MatSnackBar);
  private readonly destroyRef = inject(DestroyRef);
  private readonly wsService = inject(WebSocketService);
  private activeDialogRef: MatDialogRef<AttendanceModalComponent> | null = null;
  ...
  ngOnInit(): void {
    ...
    this.subscribeToSessionPurchases();
  }
    ...
    this.api
      ...
      .subscribe({
        next: (session) => {
          ...
          // Track active dialog to live-refresh its data on WS events
          this.activeDialogRef = dialogRef;
          this.activeOccurrenceId = occurrenceId;
  ...
  /**
   * Subscribe to real-time session purchase notifications
   *
   * Listens for WebSocket notifications when parents purchase sessions online.
   * Automatically reloads calendar to show updated session counts.
   */
  private subscribeToSessionPurchases(): void {
    this.wsService.sessionPurchase$
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (event) => {
          console.log('📢 Session purchase notification received:', event);
          this.snackbar.open(
            'Sesiuni noi cumpărate! Reîncărcare automată...',
            undefined,
            { duration: 2000 }
          );
          // Reload calendar to show updated session counts
          this.loadWeeklyCalendar();

          // If attendance modal is open, live-refresh its session data too
          if (this.activeDialogRef && this.activeOccurrenceId) {
            this.api
              .getSessionAttendance(this.activeOccurrenceId)
              .pipe(takeUntilDestroyed(this.destroyRef))
              .subscribe({
                next: (session) => {
                  this.activeDialogRef?.componentInstance.refreshSession(session);
                },
                error: () => {
                  // Ignore; modal will stay with previous data
                }
              });
          }
        },
        error: (err) => {
          console.error('❌ Error handling session purchase event:', err);
        }
      });
  }
}
