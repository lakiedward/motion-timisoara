...
import { ClubAttendanceApiService, WeeklyCalendar } from '../../services/club-attendance-api.service';
import { ClubPendingPaymentsService } from '../../services/club-pending-payments.service';
import { PaymentReportRow } from '../../../../core/models/payment.model';
import { ProductLabelPipe } from '../../../../shared/pipes/product-label.pipe';

@Component({
  selector: 'app-club-attendance-payments',
  standalone: true,
  imports: [
    CommonModule,
    MatButtonModule,
  ...
  providers: [ClubPendingPaymentsService],
  templateUrl: './club-attendance-payments.component.html',
  styleUrls: ['./club-attendance-payments.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ClubAttendancePaymentsComponent implements OnInit {
  private readonly api = inject(ClubAttendanceApiService);
  readonly clubPayments = inject(ClubPendingPaymentsService);
  ...
  readonly shouldShowPendingPaymentsSection = computed(
    () =>
      this.clubPayments.pendingPayments().length > 0 ||
      this.clubPayments.isLoadingPayments() ||
      this.clubPayments.isPaymentsError()
  );

  ngOnInit(): void {
    this.loadWeeklyCalendar();
    this.clubPayments.loadPendingPayments();
  }
  ...
  getTotalSessions(): number {
    ...
    return cal.coaches.reduce(
      (sum, coach) => sum + coach.days.reduce((daySum, day) => daySum + day.sessions.length, 0),
      0
    );
  }
  ...
  getTotalEnrolled(): number {
    ...
    return cal.coaches.reduce(
      (sum, coach) =>
        ...
        coach.days.reduce(
          (daySum, day) =>
            daySum + day.sessions.reduce((sessSum, sess) => sessSum + (sess.enrolledCount || 0), 0),
          0
        ),
      0
    );
    ...
    return cal.coaches.reduce(
      (sum, coach) =>
        ...
        coach.days
          ...
          .reduce((daySum, day) => daySum + day.sessions.length, 0),
      0
    );
  }
  ...
  isConfirming(id: string): boolean {
    return this.clubPayments.isConfirming(id);
  }

  trackPaymentById(index: number, payment: PaymentReportRow): string {
    return payment.id;
  }

  confirmCashPayment(paymentId: string): void {
    if (this.clubPayments.isConfirming(paymentId)) return;
    this.clubPayments.confirmPayment(paymentId, () => {
      this.loadWeeklyCalendar();
    });
  }

  getConfirmAriaLabel(payment: PaymentReportRow): string {
    const childName = payment.childName || 'copil';
    const isConfirming = this.clubPayments.isConfirming(payment.id);
    return isConfirming ? `Se confirmă plata cash pentru ${childName}` : `Confirmă plata cash pentru ${childName}`;
  }
        ...
        next: (session) => {
          ...
          dialogRef.afterClosed().subscribe((result) => {
            ...
            if (result.action === 'save') {
              ...
              const items = (result.children ?? []).map((c: any) => ({
  ...
  private addSessionsForChild(enrollmentId: string, count: number): void {
    this.api
      .purchaseSessions(enrollmentId, { numberOfSessions: count, paymentMethod: 'CASH' })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
          this.snackbar.open(
            `Plată cash înregistrată pentru ${count} sesiuni. Confirmă primirea banilor în lista de plăți.`,
            undefined,
            { duration: 4000 }
          );
          this.loadWeeklyCalendar();
  ...
  private bulkAddSessions(items: Array<{ enrollmentId: string; count: number }>): void {
    const reqs = items.map((i) =>
      this.api.purchaseSessions(i.enrollmentId, { numberOfSessions: i.count, paymentMethod: 'CASH' })
    );
