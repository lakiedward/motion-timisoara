...
import { ClubService, ClubLocation, CreateClubLocationRequest, UpdateClubLocationRequest } from '../../services/club.service';

type ClubLocationType = 'POOL' | 'GYM' | 'OUTDOOR' | 'TRACK' | 'OTHER';

@Component({
  selector: 'app-club-location-form',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './club-location-form.component.html',
  styleUrls: ['./club-location-form.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ClubLocationFormComponent implements OnInit, AfterViewInit {
  private readonly api = inject(ClubService);
  ...
  readonly types: { label: string; value: ClubLocationType }[] = [
    { label: 'Bazin', value: 'POOL' },
    { label: 'Sală', value: 'GYM' },
    { label: 'În aer liber', value: 'OUTDOOR' },
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    city: [''],
    type: ['OTHER' as ClubLocationType, Validators.required],
    address: [''],
    lat: [''],
    lng: ['']
  });
  ...
  readonly coordsText = computed(() => {
    const lat = this.form.get('lat')?.value?.trim();
    const lng = this.form.get('lng')?.value?.trim();
  ...
  onCancel(): void {
    void this.router.navigate(['/club/locations']);
  }
  ...
  onSubmit(): void {
    ...
    const request = id
      ? this.api.updateLocation(id, basePayload as UpdateClubLocationRequest)
      : this.api.createLocation(basePayload as CreateClubLocationRequest);

    request.pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
        this.isSaving.set(false);
        this.snackbar.open(id ? 'Locația a fost actualizată' : 'Locația a fost creată', undefined, { duration: 4000 });
        void this.router.navigate(['/club/locations']);
      },
      error: (err) => {
        this.isSaving.set(false);
      ...
      .subscribe({
        next: (locations: ClubLocation[]) => {
          ...
          this.form.patchValue({
            name: location.name,
            city: location.city || '',
            type: (location.type as ClubLocationType) || 'OTHER',
  ...
  private initMap(): void {
    ...
    const el = document.getElementById('club-location-form-map');
    if (!L || !el) {
      return;
    }
  ...
  getTypeLabel(type: ClubLocationType | null | undefined): string {
    if (!type) return '';
    const typeMap: Record<ClubLocationType, string> = {
      POOL: 'Bazin',
      GYM: 'Sală',
