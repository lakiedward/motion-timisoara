...
import { getMyLocations, setLocationActive } from '@/api/coach'
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold text-foreground">Locațiile mele</h1>
        <Button asChild>
          <Link to="/coach/locations/new">
            <Plus /> Locație nouă
          </Link>
        </Button>
      </div>
      ...
      {isLoading ? (
            ...
            <div key={l.id} className="bg-card shadow-card rounded-3xl p-5">
              ...
              <div className="mt-4 flex gap-2">
                <Button asChild size="sm" variant="outline">
                  <Link to={`/coach/locations/${l.id}/edit`}>
                    <Pencil /> Editează
                  </Link>
                </Button>
        ...
        <div className="text-muted-foreground rounded-3xl border border-dashed py-16 text-center">
          Nicio locație încă.{' '}
          <Link to="/coach/locations/new" className="text-primary font-semibold">
