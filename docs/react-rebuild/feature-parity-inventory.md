...
2. **Stripe Connect onboarding redirects (coach + club)** — Full-page redirects to Stripe and back to return/refresh pages that live *outside* auth guards (user returns unauthenticated-ish from Stripe). Connect destination resolution (`payment_recipient` + account-enabled flags + COACH↔CLUB fallback) and platform-fee math (1.19%) must match `_shared/stripe.ts` exactly. Onboarding link expiry (15 min) handling.
