...
export function cashPaymentHandler({
  ...
  return async (req: Request) => {
    ...
    if (!["COACH", "CLUB", "ADMIN"].includes(role ?? "")) {
      return enrollmentJson(
        { error: "Nu ai acces la confirmarea plăților." },
        403,
      );
    }
    const { paymentId } = await req.json();
    if (typeof paymentId !== "string" || !paymentId) {
      return enrollmentJson({ error: "Plata nu este validă." }, 400);
    }
    const { data: payment, error } = await db
      .from("payments")
      .select(
        "id, amount, currency, method, enrollment:enrollments(kind, entity_id)",
      )
      .eq("id", paymentId)
      .single();
    if (error || !payment) {
      return enrollmentJson({ error: "Nu am putut găsi plata." }, 404);
    }
    if (payment.method !== "CASH") {
      return enrollmentJson(
        { error: "Poți confirma aici doar plățile cash." },
        400,
      );
    }
    ...
    if (!enrollment) {
      return enrollmentJson({ error: "Înscrierea nu a fost găsită." }, 404);
    }
    if (role !== "ADMIN") {
      const table = {
        COURSE: "courses",
      ...
      if (!table) {
        return enrollmentJson({ error: "Tip de înscriere necunoscut." }, 400);
      }
      ...
      if (offerError) {
        return enrollmentJson(
          { error: "Nu am putut verifica organizatorul." },
          503,
        );
      }
      ...
      if (!owned) {
        return enrollmentJson(
          { error: "Poți confirma doar plățile ofertelor tale." },
          403,
        );
      }
    }
    ...
    if (result.status !== "SUCCEEDED") {
      return enrollmentJson(
        { error: "Plata nu mai poate fi confirmată." },
        409,
      );
    }
