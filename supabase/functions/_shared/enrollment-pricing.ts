...
import { createPriceSnapshot, type PriceSnapshot, readPriceSnapshot } from "./price-snapshot.ts";
...
export function validSelection(
  ...
  if (
    !["CAMP", "COURSE", "ACTIVITY"].includes(String(kind)) ||
...
export function quoteSubjectKey(
  quote: Pick<EnrollmentChildPrice, "childId" | "adultProfileId">,
) {
  return quote.childId ?? quote.adultProfileId ?? "";
}

async function priceVersion(
  campId: string,
  childId: string,
  amount: number,
  currency: string,
  ...
  const bytes = new TextEncoder().encode(
    JSON.stringify([campId, childId, amount, currency]),
  );
...
export async function getEnrollmentChildPrices(
  ...
  if (pendingIds.length) {
    const result = await db
      .from("payments")
      .select(
        "id, enrollment_id, amount, currency, status, gateway_txn_id, pricing_snapshot",
      )
      .in("enrollment_id", pendingIds);
    if (result.error) {
      throw enrollmentJson(
        { error: "Nu am putut verifica plățile existente." },
        500,
      );
    }
  ...
  return await Promise.all(
    childIds.map(async (childId) => {
      ...
      const childPayments = payments.filter((row) =>
        enrollments.some((enrollment) => enrollment.id === row.enrollment_id)
      );
      if (childPayments.length > 1 || enrollments.length > 1) {
        return {
          childId,
          reason: "Înscrierea are nevoie de verificare. Contactează clubul.",
        };
      }
      ...
      if (
        ...
        return {
          childId,
          reason: "Plata a fost deja procesată. Verifică în Înscrieri.",
        };
      }
      ...
      if (payment?.pricing_snapshot) {
        ...
        if (
          ...
          throw enrollmentJson(
            { error: "Oferta salvată nu corespunde înscrierii." },
            500,
          );
        }
        if (snapshot.quantity !== quantity) {
          return {
            childId,
            reason:
              `Pachetul salvat are ${snapshot.quantity} ședințe. Alege acest număr pentru a relua plata confirmată inițial.`,
          };
        }
        return {
          childId,
      ...
      if (payment?.gateway_txn_id) {
        if (
          ...
          throw enrollmentJson(
            {
              error: "Plata existentă nu este în lei sau suma nu este validă. Contactează clubul.",
            },
            409,
          );
        }
        return {
          ...
          currency: "RON",
          payment,
          priceVersion: await priceVersion(
            `${kind}:${entityId}:${payment.id}`,
            childId,
            payment.amount,
            payment.currency,
          ),
      ...
      if (kind === "CAMP") {
        const result = await db.rpc("enrollment_camp_offer", {
          p_camp_id: entityId,
          p_child_id: childId,
        });
        if (result.error) {
          throw enrollmentJson(
            { error: "Nu am putut calcula prețul taberei. Încearcă din nou." },
            500,
          );
        }
        if (result.data === null || result.data.amount === null) {
          return {
            childId,
            reason: "Nu există o categorie de preț pentru vârsta copilului la începutul taberei.",
          };
        }
      ...
      try {
        ...
        throw enrollmentJson(
          {
            error: "Prețul sau cursul valutar nu este valid. Contactează clubul.",
          },
          500,
        );
      }
...
export async function getEnrollmentAdultPrice(
  ...
  const enrollments = existing.filter(
    (row) => row.adult_profile_id === parentId,
  );
  if (enrollments.some((row) => row.status === "ACTIVE")) {
  ...
  if (payment?.pricing_snapshot) {
    ...
    if (
      snapshot.kind !== "CAMP" ||
      snapshot.entityId !== entityId ||
      snapshot.adultProfileId !== parentId ||
      snapshot.amount !== payment.amount ||
      snapshot.currency !== payment.currency
    ) {
      throw enrollmentJson(
  ...
  const result = await db.rpc("enrollment_camp_adult_offer", {
    p_camp_id: entityId,
  });
  if (result.error) {
    throw enrollmentJson(
      { error: "Nu am putut calcula prețul adult. Încearcă din nou." },
      500,
    );
  }
  if (result.data === null || result.data.amount === null) {
    return {
      adultProfileId: parentId,
      reason: "Tabăra nu are un tarif adult.",
    };
  }
  try {
    const snapshot = await createPriceSnapshot(
      ...
      result.data.eur_ron_rate_micros ?? null,
      1,
      parentId,
    );
    return {
