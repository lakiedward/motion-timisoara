...
export async function applyPaymentResult(db: SupabaseClient, input: {
  ...
  if (
    error || !data || typeof data.changed !== "boolean" ||
    typeof data.sessionsAdded !== "number"
  ) {
...
export async function notifyPaymentResult(
  ...
  try {
    const { data: enrollment, error } = await db.from("enrollments")
      .select(
        "id, entity_id, child_id, adult_profile_id, child:children(parent_id)",
      )
      .eq("id", result.enrollmentId).single();
    ...
    if (result.sessionsAdded > 0) {
      await broadcast("admin:session-purchases", "session_purchase", {
        ...
        sessionCount: result.sessionsAdded,
        courseId: enrollment.entity_id,
      });
    }
    const child = enrollment.child as unknown as { parent_id: string } | null;
    const recipientId = enrollment.child_id
      ? child?.parent_id
      : enrollment.adult_profile_id;
    if (recipientId) {
      await broadcast(
        `user:${recipientId}:payments`,
        result.status === "SUCCEEDED" ? "enrollment_ready" : "payment_failed",
        result.status === "SUCCEEDED"
          ? { enrollmentId: result.enrollmentId, status: "ACTIVE" }
          : {
            enrollmentId: result.enrollmentId,
            reason: failureReason ?? "Plata a eșuat. Poți încerca din nou.",
          },
      );
    }
  } catch {
    console.error(
      "Payment saved; realtime notification could not be delivered",
    );
  }
}
