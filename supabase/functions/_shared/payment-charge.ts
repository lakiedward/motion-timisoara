...
export async function authorizedRonCharge(
  ...
  if (
    ...
    throw enrollmentJson(
      {
        error: "Plata nu poate fi inițiată. Verifică suma în lei și starea înscrierii.",
      },
      409,
    );
  }
    ...
    try {
      ...
      if (enrollment.kind === "COMPETITION") {
        ...
        const { data: registration, error: registrationError } = await db
          ...
          .select(
            "competition_id,category_id,route_id,gpx_storage_path_snapshot,accepted_price_bani",
          )
