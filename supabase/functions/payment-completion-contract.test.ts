...
function database(kind = "COURSE", role = "COACH", own = true) {
  ...
  const payment = {
    ...
    currency: "RON",
    method: "CASH",
    enrollment: { kind, entity_id: "offer" },
  };
  const rows: Record<string, Record<string, unknown> | null> = {
    ...
    competitions: { coach_id: own ? "actor" : "foreign", club_id: "club" },
  ...
  const client = {
      ...
      return {
        ...
        single() {
          ...
          return Promise.resolve({
            data: failedTable === table ? null : rows[table],
            error: failedTable === table ? { message: "offline" } : null,
          });
        },
      };
    },
    rpc(name: string, args: Record<string, unknown>) {
      ...
      return Promise.resolve({
        ...
        error: rpcError ? { message: "offline" } : null,
      });
    },
  };
  return {
...
const cashRequest = () =>
  new Request("http://local/mark-cash-paid", {
    method: "POST",
    body: JSON.stringify({ paymentId: "payment" }),
  });

for (const role of ["COACH", "CLUB", "ADMIN"]) {
  for (const kind of ["COURSE", "ACTIVITY", "CAMP", "COMPETITION"]) {
    Deno.test(
      `cash confirmation: ${role} can confirm owned ${kind} without reading current price`,
      async () => {
        const services = database(kind, role);
        const response = await cashPaymentHandler(services)(cashRequest());
        equal(response.status, 200);
        equal(services.calls, [
          {
            name: "apply_enrollment_payment_result",
            args: {
    ...
    if (role !== "ADMIN") {
      Deno.test(
        `cash confirmation: ${role} cannot confirm foreign ${kind}`,
        async () => {
          const services = database(kind, role, false);
          equal(
            (await cashPaymentHandler(services)(cashRequest())).status,
            403,
          );
...
Deno.test(
  "cash endpoint refuses card payments and ownership read failures",
  async () => {
...
Deno.test(
  "cash confirmation cannot return success after transaction failure",
  async () => {
    const services = database();
    services.failRpc();
    const response = await withCors(cashPaymentHandler(services))(
      cashRequest(),
    );
...
Deno.test(
  "webhook verifies the raw signature before touching payment state",
  async () => {
    const services = database();
...
for (
  const [type, outcome] of [
    ["payment_intent.succeeded", "SUCCEEDED"],
    ["payment_intent.payment_failed", "FAILED"],
  ]
) {
  Deno.test(
    `webhook delegates ${outcome} atomically with the exact gateway charge`,
    async () => {
      const services = database();
...
Deno.test("webhook does not acknowledge a failed transaction", async () => {
  ...
  services.failRpc();
  equal(
    (await webhook(services.db).handler(webhookRequest("valid"))).status,
    500,
  );
});

Deno.test(
  "webhook refuses a partial received amount and ignores unrelated events",
  async () => {
    const services = database();
    equal(
      (
        await webhook(services.db, "payment_intent.succeeded", 49999).handler(
          webhookRequest("valid"),
        )
      ).status,
      409,
    );
    equal(
      (
        await webhook(services.db, "customer.created").handler(
          webhookRequest("valid"),
        )
      ).status,
      200,
