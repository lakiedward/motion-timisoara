CREATE FUNCTION public.save_enrollment_batch(
    p_parent_id UUID,
    p_kind TEXT,
    p_entity_id UUID,
    p_method TEXT,
...
BEGIN
    ...
    LOOP
        ...
        IF v_exists THEN
            ...
            IF v_enrollment.status = 'ACTIVE' THEN
                IF v_payment.id IS NULL OR v_payment.status <> 'SUCCEEDED' OR v_payment.method <> 'CARD'
                    OR v_payment.gateway_txn_id IS NULL OR v_snapshot IS NULL
                    OR v_payment.pricing_snapshot IS DISTINCT FROM v_snapshot
                    OR v_payment.amount IS DISTINCT FROM v_amount OR v_payment.currency <> 'RON' THEN
    ...
    RETURN jsonb_build_object('enrollmentId',v_ids->>0,'enrollmentIds',v_ids,'createdEnrollmentIds',v_created,
        'prices',v_prices,'requiresPaymentIntent',p_method='CARD');
END;
$$;
