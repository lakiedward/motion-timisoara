...
BEGIN
    ...
    IF NOT FOUND OR enrollment.kind IS DISTINCT FROM 'COMPETITION'
        ...
        OR enrollment.child_id IS NULL THEN
...
BEGIN
    IF p_snapshot IS NULL
        ...
        OR NOT p_snapshot ?& ARRAY[
            'schemaVersion', 'kind', 'entityId', 'childId', 'categoryId',
        ...
        OR (p_snapshot->>'childId' ~ '^[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}$') IS DISTINCT FROM TRUE
    ...
    child_id := (p_snapshot->>'childId')::UUID;
    ...
    RETURN price_bani BETWEEN 0 AND 99999999
        ...
        AND p_snapshot->>'priceVersion' = public.competition_price_version(
            competition_id, child_id, category_id, route_id, gpx_storage_path, price_bani
        );
...
BEGIN
    ...
    IF jsonb_array_length(p_quotes) = 0 THEN
        RAISE EXCEPTION 'Selectează cel puțin un copil'
            USING ERRCODE = '22023';
    END IF;
    IF EXISTS (
        SELECT 1 FROM jsonb_array_elements(p_quotes) item
        WHERE jsonb_typeof(item) IS DISTINCT FROM 'object'
           OR (item->>'childId' ~ '^[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}$') IS DISTINCT FROM TRUE
    ...
    END IF;
    IF (
        SELECT count(DISTINCT item->>'childId')
        FROM jsonb_array_elements(p_quotes) item
    ) <> jsonb_array_length(p_quotes) THEN
        RAISE EXCEPTION 'Același copil nu poate fi selectat de două ori'
            USING ERRCODE = '22023';
    END IF;

    PERFORM 1 FROM public.profiles profile
    WHERE profile.id = p_parent_id AND profile.role = 'PARENT'
    FOR SHARE;
    IF NOT FOUND THEN
    ...
    LOOP
        ...
        age_at_registration := public.varsta_la_data(child.birth_date, registration_date);
        IF age_at_registration NOT BETWEEN category.age_from AND category.age_to THEN
            RAISE EXCEPTION 'Vârsta copilului nu corespunde categoriei alese'
                USING ERRCODE = '23514';
        END IF;
        ...
        INSERT INTO public.competition_registrations (
            ...
            age_at_registration, accepted_price_bani, registered_at
        ) VALUES (
            enrollment_id, p_competition_id, category.id, route.id,
            category.name, route.name, route.gpx_storage_path,
            age_at_registration, price_bani, registration_at
        );
