...
$containerName = 'motion-camp-rules-file-' + [Guid]::NewGuid().ToString('N')
...
try {
    ...
    Invoke-RulesFileDocker @('cp', (Join-Path $PSScriptRoot 'camp-rules-file.sql'), "${containerName}:/tmp/test.sql")
