...
$containerName = 'motion-price-snapshots-' + [Guid]::NewGuid().ToString('N')
...
try {
    ...
    Invoke-PricingDocker @('cp', (Join-Path $PSScriptRoot '../migrations/00048_enrollment_price_snapshots.sql'), "${containerName}:/tmp/migration.sql")
    Invoke-PricingDocker @('cp', (Join-Path $PSScriptRoot 'enrollment-price-snapshots.sql'), "${containerName}:/tmp/test.sql")
